...
class DofNota extends Model
{
    ...
    public function scopeContains($query, $clave){
    	return $query->select(DB::raw("cod_nota, CASE WHEN entity2char(titulo)  ~* '".$clave. "' THEN 'Título' ELSE 'Contenido' END AS ubicacion, (regexp_matches(entity2char(titulo || ' '|| contenido), '".$clave."', 'gi'))[1] as mencion"));
    }
