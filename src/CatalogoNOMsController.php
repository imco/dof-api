...
class CatalogoNOMsController extends Controller {
	...
	public function getNomsListByDependency($dependencia = NULL){
		...
		$result = DB::connection($this->connection)->select(DB::connection($this->connection)->raw($sqlQuery));
	...
	function getNOMPublications($clave) {
		...
		$historial = DB::connection($this->connection)->select(DB::connection($this->connection)->raw("
	...
	function getNOM($clave) {
		...
		$historial = DB::connection($this->connection)->select(DB::connection($this->connection)->raw("SELECT  fecha,cod_nota, clavenomnorm, etiqueta, entity2char(titulo), urlnota AS url
		...
		$ramayProducto = DB::connection($this->connection)->select(DB::connection($this->connection)->raw("
		...
		$comite = DB::connection($this->connection)->select(DB::connection($this->connection)->raw("SELECT  secretaria, nombre_secretaria, comite, descripcion_comite from comite WHERE :clavenomnorm like ('%'||comite||'%')ORDER BY secretaria, comite ASC;"),
			array('clavenomnorm' => '%' . substr($clave, 3, -4) . '%'));
